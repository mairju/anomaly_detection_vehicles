...
class PoissonBlending:
    ...
    def apply(self, placement_image, dest_img, placement_mask):
        pcomp_result_initial = np.dstack([
            self.compute_poisson_blend_channel(placement_image[:, :, c]/255., dest_img[:, :, c]/255., placement_mask, mode='normal')
            for c in range(3)
        ])
        
        pcomp_result = self.apply_clahe_mask(pcomp_result_initial, placement_mask)
